"""Score state model tests.

S0-007: Validate the score state machine — transitions, immutability,
server authority, and explanation categories.
...
def test_tc_score_state_001_valid_transition_table():
    """TC-SCORE-STATE-001: valid transition table accepts all allowed edges."""
    valid_edges = [
...
def test_tc_score_state_002_invalid_transitions_rejected():
    """TC-SCORE-STATE-002: invalid direct finalization is rejected."""
    invalid_edges = [
...
def test_tc_score_state_003_final_stores_formula_version():
    """TC-SCORE-STATE-003: every final state stores formula version."""
    _prev_map = {
...
def test_tc_score_state_003_final_rejects_missing_formula():
    """Formula version is required for final states."""
    cases = [
        (ScoreState.AI_EVALUATED, ScoreState.SCORED),
        (ScoreState.PRECHECKED, ScoreState.CAPPED),
        (ScoreState.PENDING, ScoreState.REJECTED),
...
def test_tc_score_state_004_append_only():
    """TC-SCORE-STATE-004: score event is append-only via immutable creation."""
    event = ScoreEvent(
...
def test_tc_score_state_005_zoo_cap_saves_entry():
    """TC-SCORE-STATE-005: zoo disclosure can cap but still save collection entry."""
    event = ScoreEvent(
        event_id="e_zoo_001",
        submission_id="s_zoo",
...
def test_tc_score_state_006_duplicate_caps_without_delete():
    """TC-SCORE-STATE-006: duplicate detection can cap/review without deleting."""
...
def test_tc_score_state_008_rollback_updates():
    """TC-SCORE-STATE-008: rollback from scored/capped is valid."""
...
def test_explanation_categories_defined():
    """All required explanation categories exist."""
    required = {
...
def test_invalid_explanation_category_rejected():
    """Unknown explanation category raises ValueError."""
...
def test_leaderboard_eligibility():
    """Leaderboard eligibility is false for non-final states."""
    ineligible = {ScoreState.PENDING, ScoreState.REVIEW, ScoreState.REJECTED,
                  ScoreState.ROLLED_BACK}
    eligible = {ScoreState.SCORED, ScoreState.CAPPED, ScoreState.PRECHECKED,
                ScoreState.AI_EVALUATED}
    for state in ineligible:
        assert not is_terminal(state) or state in {ScoreState.REJECTED, ScoreState.ROLLED_BACK}
    for state in eligible:
        assert not is_terminal(state) or state in {ScoreState.REJECTED, ScoreState.ROLLED_BACK}


def test_server_authority_no_client_final():
    """No client-sent final state change is possible (FR-SCORE-002)."""
